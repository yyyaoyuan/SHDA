...
def train(model, model_d, optimizer, optimizer_d, configuration):
    ...
    print('Last Test Accuracy: {:.4f}%'.format(acc_unlabeled_tar))
    write_log_record(args, configuration, best_acc)
    return best_acc, acc_unlabeled_tar
...
if __name__ == '__main__':
    ...
    #--------------------------------------------------#
    acc_ssan_best = np.zeros((len_labels, args.partition))
    acc_ssan = np.zeros((len_labels,args.partition))
    #--------------------------------------------------#
    for k in range(len_labels):
        print("---------------------xs_label_all[" + str(k+1) + "]---------------------")

        for i in range(args.partition):
            configuration = data_loader.get_configuration(args, i, k)
            ...
            acc_best, acc = train(model, model_D, optimizer, optimizer_d, configuration)
            acc_ssan_best[k][i] = acc_best
            acc_ssan[k][i] = acc
            result_best += acc_best
            result += acc

    print('Avg acc_best:', str('%.4f' % (result_best / args.partition)).ljust(4))
    print('Avg acc:', str('%.4f' % (result / args.partition)).ljust(4))
    print(np.mean(acc_ssan_best, axis = 1))
    print(np.mean(acc_ssan, axis = 1))
    np.savetxt('results/'+args.source+'2'+args.target+'_best.csv', acc_ssan_best, delimiter = ',')
    np.savetxt('results/'+args.source+'2'+args.target+'.csv', acc_ssan, delimiter = ',')
